...
```bash
docker pull ghcr.io/octoverse-id/octonomy:3.2.1
```
...
```bash
gh attestation verify oci://ghcr.io/octoverse-id/octonomy:3.2.1 \
...
`--source-ref refs/tags/v3.2.1` (needs `gh` 2.68+). `:edge` is **not** attested and fails all of
this by design.
