...
#
# The destination is a MIRROR of the live rulesets, not an append-only pile. A ruleset
# that was deleted or renamed upstream has its old file removed, because a stale document
# sitting in this directory reads as current policy and the restore procedure in the
# README would happily re-apply something nobody wants any more.
#
...
#
# Exit codes: 0 ok | 1 export would be lossy (slug collision) | 2 usage
#             | 3 cannot read rulesets
...
# Stage into a temp dir and only touch the destination once every ruleset exported
# cleanly. A run that dies halfway must not leave the record half-updated.
staging=$(mktemp -d)
...
for id in $ids; do
  ...
  [ -n "$slug" ] || die "ruleset $id ('$name') normalises to an empty file name" 3

  # Two different names can normalise to one slug — "Foo bar" and "Foo-bar" both give
  # foo-bar. Overwriting would drop a policy from the record while the summary still
  # counted it, so refuse instead: a record missing a ruleset is worse than no record.
  if [ -n "${slug_owner[$slug]:-}" ]; then
    echo "FAIL  '$name' (id $id) and '${slug_owner[$slug]}' both normalise to $slug.json" >&2
    die "rename one ruleset so each has a distinct file name — refusing to write a record that omits one" 1
  fi
  slug_owner[$slug]=$name

  printf '%s\n' "$body" | jq -S "$FILTER" >"$staging/$slug.json"
  count=$((count + 1))
done

# Reconcile: drop files for rulesets that no longer exist (deleted or renamed), then
# write the current set. Only *.json is managed here — README.md and anything else a
# human put in this directory is left alone.
removed=0
for existing in "$out_dir"/*.json; do
  [ -e "$existing" ] || continue
  if [ ! -e "$staging/$(basename "$existing")" ]; then
    rm -f "$existing"
    echo "removed  $existing (no longer a live ruleset)"
    removed=$((removed + 1))
  fi
done

for staged in "$staging"/*.json; do
  cp "$staged" "$out_dir/$(basename "$staged")"
  echo "exported $out_dir/$(basename "$staged")"
done

echo "export-rulesets OK: $count ruleset(s) written to $out_dir, $removed stale file(s) removed"
