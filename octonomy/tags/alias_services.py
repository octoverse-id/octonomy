...
def reactivate_tag_alias(alias: TagAlias, audit_context: AuditContext | None = None) -> TagAlias:
    ...
    re-validates the tag with the tag row locked inside the write transaction. The lock
    order is ALWAYS tag -> alias (matching deactivate_tag's cascade), so a concurrent
    ...
    raise ConflictError(
        "The alias's canonical tag kept changing during reactivation; retry.",
        {"tag_id": ["Alias was re-pointed concurrently; retry."]},
    )
